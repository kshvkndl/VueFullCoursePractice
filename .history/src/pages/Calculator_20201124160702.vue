<template>
  <section class="flex w-full">
    <div class="m-auto">
      ...
      <p
        class="text-2xl text-right mt-5 mb-3 border rounded shadow w-45 h-10 p-1 overflow-x-hidden"
        style="direction: rtl"
      >
        {{ calculation }}
      </p>
      <div class="mb-10 grid grid-cols-4 gap-4">
        ...
        <button
          ...
          class="p-2 border shadow h-10 rounder col-span-2"
        >
          0
        </button>
        <button
...
export default {
  setup() {
    ...
    function buttonPressed(value) {
      if (value === "=") calculate();
      else if (operations.includes(value)) applyOperation();
      else appendNumber();
      calculation.value = calculation.value + value;
    }

    return { calculation, buttonPressed };
  },
};
</script>
